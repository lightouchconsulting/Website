...
export async function scrapeFeeds(feedUrls: string[], cutoffDays = 7): Promise<Article[]> {
  ...
  const settled = await Promise.allSettled(feedUrls.map(async (url) => {
    ...
    for (const item of feed.items ?? []) {
      ...
      articles.push({ title, link, source: sourceName, snippet, pubDate: pubDate?.toISOString() ?? '' })
    }
    return articles
  }))
  ...
  for (const [i, result] of settled.entries()) {
    if (result.status === 'fulfilled') {
      ...
      console.warn(`[scraper] Failed to fetch ${feedUrls[i]}:`, result.reason?.message ?? result.reason)
    }
  }
